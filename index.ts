...
async function getZoneId(domain: string): Promise<string> {
    const response = await fetch(`${ZONE_URL}?name=${domain}`, { headers: HEADERS });
    if (!response.ok) throw new Error(`Failed to retrieve zone ID for domain: ${domain}`);
...
async function selectSubdomainsToUpdate(zoneId: string, domain: string) {
    ...
    if (!response.ok) throw new Error(`Failed to fetch DNS records for domain: ${domain}`);
    ...
    const selectedIndices = await prompt("Enter the numbers of the subdomains to update (comma-separated): ");
...
async function testDnsRecords(zoneId: string, domain: string, currentIp: string) {
    const dnsRecordsUrl = `${ZONE_URL}/${zoneId}/dns_records?type=A`;
    const response = await fetch(dnsRecordsUrl, { headers: HEADERS });
    if (!response.ok) throw new Error(`Failed to fetch DNS records for domain: ${domain}`);
    ...
    for (const record of records) {
        if (record.name.endsWith(`.${domain}`) && config.selectedSubdomains[domain]?.includes(record.name)) {
...
async function updateDnsRecords(zoneId: string, domain: string, newIp: string) {
    ...
    for (const record of records) {
        if (record.name.endsWith(`.${domain}`) && config.selectedSubdomains[domain]?.includes(record.name) && record.content !== newIp) {
